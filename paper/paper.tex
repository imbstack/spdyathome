...
\S\ref{sec:intro/spdy} and then the remainder of the paper will focus on
...
This project sets out to answer that question in two fashions.  First with an
in-depth examination of what is the limiting factor of current HTTP/1.1
including a review of literature in the area and a characterization of
real-world web page asset size distribution (and how we predict the same assets
will load if loaded over SPDY).  Then, we will collect direct measurements with
a simulated browser experiment to determine empirically how much of an
improvement is seen when using SPDY rather than HTTP.  The experiment is
proposed in \S\ref{sec:project}.
